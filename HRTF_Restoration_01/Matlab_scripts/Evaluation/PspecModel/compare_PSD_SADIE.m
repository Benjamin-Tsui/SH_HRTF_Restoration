function [PSD_in_summary,PSD_out_summary, PSD_in_angle, PSD_out_angle] = compare_PSD_SADIE(subject,model, plotflag)
...
disp(['SADIE subject ' subject])

PSD_mean = weightedAmbiSpecDiffLandR;
disp(strcat('mean PSD (input) = ',num2str(weightedAmbiSpecDiffLandR)))
...
%         fill3(X(1,:),X(2,:),X(3,:),s(k),'Parent',ax,'EdgeColor','none');     % for no edge colour / outline
    ...
    title(['subject  ' subject '  (input), mean PSD = ' num2str(PSD_mean)])
...
if plotflag == 2 || plotflag == 3
    ...
    title(['\fontsize{21}subject  ' subject '  (input), mean PSD = ' num2str(PSD_mean)])
...
PSD_mean = weightedAmbiSpecDiffLandR;
disp(strcat('mean PSD (reconstructed) = ',num2str(weightedAmbiSpecDiffLandR)))
...
%         fill3(X(1,:),X(2,:),X(3,:),s(k),'Parent',ax,'EdgeColor','none');     % for no edge colour / outline
    ...
    title(['subject ' subject ' (reconstructed), mean PSD = ' num2str(PSD_mean)])
...
if plotflag == 2 || plotflag == 3
    ...
    figure
    heatmap_plot(angle_matched(:,1)',angle_matched(:,2)',PSD_angle)
    title(['\fontsize{21}subject ' subject ' (reconstructed), mean PSD = ' num2str(PSD_mean)])
    xlabel('Azimuth (°)','FontSize',18)
    ylabel('Elevation (°)','FontSize',18)
    colormap(flipud(parula));
    c2 = colorbar;
